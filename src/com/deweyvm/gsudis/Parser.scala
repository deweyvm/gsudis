...
class HalfOp(op:String, code:Char, f:Char=>Boolean=(x=>true), reqState:AltState=AltNone, newState:AltState= AltNone)(p:OpPrinter) extends Parser(op, reqState, newState, p) {
...
class RegArgOp(op:String, code:Char, reqState:AltState=AltNone, newState:AltState= AltNone)(p:OpPrinter) extends Parser(op, reqState, newState, p) {
  def myProcess(input:Vector[Byte]) = input match {
...
//half byte opcode, half byte register, one byte arg
class RegArg2Op(op:String, code:Char, reqState:AltState=AltNone, newState:AltState= AltNone)(p:OpPrinter) extends Parser(op, reqState, newState, p) {
  def myProcess(input:Vector[Byte]) = input match {
...
class WordOp(op:String, code:Byte, reqState:AltState=AltNone, newState:AltState=AltNone)(p:OpPrinter) extends Parser(op, reqState, newState, p) {
  def myProcess(input:Vector[Byte]) = input match {
...
class ArgOp(op:String, code:Byte, reqState:AltState=AltNone, newState:AltState=AltNone)(p:OpPrinter) extends Parser(op, reqState, newState, p) {
  def myProcess(input:Vector[Byte]) = input match {
...
class MoveOp(op:String, code1:Char, code2:Char, reqState:AltState=AltNone, newState:AltState=AltNone)(p:OpPrinter) extends Parser(op, reqState, newState, p) {
  def myProcess(input:Vector[Byte]) = input match {
